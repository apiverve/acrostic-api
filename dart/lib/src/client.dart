...
class AcrosticValidationException implements Exception {
  final List<String> errors;

  AcrosticValidationException(this.errors);

  @override
  String toString() => 'AcrosticValidationException: ${errors.join("; ")}';
}
...
/// Acrostic Puzzle Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/acrostic?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [word] (required) - The keyword to spell out (3-15 letters)
/// * [theme] - Theme: random, positive, nature, adventure, friendship, learning
class AcrosticClient {
  final String apiKey;
  ...
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw AcrosticValidationException(errors);
    }
  }

  /// Execute a request to the Acrostic Puzzle Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<AcrosticResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw AcrosticException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return AcrosticResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw AcrosticException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw AcrosticException('Resource not found');
      } else {
        throw AcrosticException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is AcrosticException) rethrow;
      throw AcrosticException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Acrostic Puzzle Generator API client.
class AcrosticException implements Exception {
  final String message;

  AcrosticException(this.message);

  @override
  String toString() => 'AcrosticException: $message';
}
